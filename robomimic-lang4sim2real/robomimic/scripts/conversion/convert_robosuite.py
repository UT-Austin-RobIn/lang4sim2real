...
if __name__ == "__main__":
    ...
    if args.multitask_hdf5_format:
        for task_idx in f["data"]:
            # ensure model-xml is in per-episode metadata
            for ep in f[f"data/{task_idx}"]:
                assert "model_file" in f[f"data/{task_idx}/{ep}"].attrs

                # add "num_samples" into per-episode metadata
                if "num_samples" in f[f"data/{task_idx}/{ep}"].attrs:
                    del f[f"data/{task_idx}/{ep}"].attrs["num_samples"]
                n_sample = f[f"data/{task_idx}/{ep}/actions"].shape[0]
                f[f"data/{task_idx}/{ep}"].attrs["num_samples"] = n_sample
                total_samples += n_sample
    else:
        for ep in f["data"]:
            print("Attributes:", f["data/{}".format(ep)].attrs)
            # ensure model-xml is in per-episode metadata
