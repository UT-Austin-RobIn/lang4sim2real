...
def split_train_val_from_hdf5(
        hdf5_path, val_ratio=0.1, multitask=False, filter_key=None):
    """
    ...
    if multitask:
        train_keys = []
        valid_keys = []
        for task_idx in list(f["data"].keys()):
            demos = list(f["data"][task_idx].keys())
            mask = np.zeros(len(demos))
            mask[:int(val_ratio * len(demos))] = 1.
            np.random.shuffle(mask)
            mask = mask.astype(int)
            train_inds = (1 - mask).nonzero()[0]
            valid_inds = mask.nonzero()[0]
            train_keys.extend([f"{task_idx}{demos[i]}" for i in train_inds])
            valid_keys.extend([f"{task_idx}{demos[i]}" for i in valid_inds])
    ...
    train_lengths = create_hdf5_filter_key(
        ...
        key_name=name_1,
        multitask_hdf5_format=multitask)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multitask", action="store_true", default=False
    )
