...
class EnvRobosuite(EB.EnvBase):
    ...
    def reset_to(self, state):
        ...
        if "model" in state:
            reset_kwargs = {}
            if self.env.is_mm_env:
                reset_kwargs = {"no_mm_metric_reset": True}
        ...
        if "goal" in state:
            self.set_goal(task_id=state["goal"])
        if should_ret:
            # only return obs if we've done a forward call - otherwise the observations will be garbage
            obs = self.get_observation()
            if self.env.is_mm_env:
                self.env.update_metrics_for_mm(obs)
            return obs
        return None
    ...
    def get_observation(self, di=None):
        ...
        # "task_id" key contains task_id info for multitask envs.
        ret["task_id"] = np.array(di["task_id-state"])

        if hasattr(self.env, "mm_state_keys"):
            for mm_key in self.env.mm_state_keys:
                ret[mm_key] = np.array(di[mm_key])
