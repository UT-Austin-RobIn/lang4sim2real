...
class BCConfig(BaseConfig):
    ...
    def algo_config(self):
        ...
        # vars for baseline (ground truth task embs)
        self.algo.gt_task_emb = ""                       # when we don't want to learn the task-embs, what kind of task emb we pass in.

        # Verb Obj encoder settings
        self.algo.verb_obj_enc.enabled = True            # whether to train Verb and Object encoders
        self.algo.verb_obj_enc.sample_size = 10          # num timesteps to sample evenly from a trajectory for verb/obj encoder obs.
        self.experiment.rollout.target.data = ""         # path to target task demos (only for producing embs during rollout)
        self.algo.verb_obj_enc.task_emb_mode = "lang"    # Either ["lang", "onehot"]. for targets we regress toward.
        self.algo.verb_obj_enc.loss = "contrastive"      # Either ["contrastive", "cosdist"]. for task encoder loss.
        self.algo.verb_obj_enc.task_enc_contr_temp = 1.0 # temp to divide dot product in contrastive learning
        self.algo.verb_obj_enc.task_enc_weight = 1.0     # weight on task encoder loss terms
