...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        load_next_obs=True,
        load_verb_obj_enc_batch=False,
        verb_obj_id_to_task_id_map=None,
        ...
        Args:
            ...
            load_verb_obj_enc_batch (bool): whether to load batches for verb and obj encoders

            verb_obj_id_to_task_id_map (dict): mapping (verb_id, obj_id) --> task_id. Only used if using language in Multitaskv1
        """
        ...
        self.load_verb_obj_enc_batch = load_verb_obj_enc_batch
        self.verb_obj_id_to_task_id_map = verb_obj_id_to_task_id_map
        if self.verb_obj_id_to_task_id_map is not None:
            self.env_num_tasks = len(self.verb_obj_id_to_task_id_map)
    ...
    def get_item(self, index):
        ...
        # Load verb/obj/task_ID info
        task_id_verbobj_onehot = self.get_dataset_for_ep(demo_id, "obs/task_id")[0] # 0th timestep, since task_id doesn't change during course of demo
        verb_id_onehot = task_id_verbobj_onehot[:4] # TODO: Fix hardcoding
        assert set(verb_id_onehot) == {0, 1} and np.sum(verb_id_onehot) == 1
        meta["verb_ids"] = np.argmax(verb_id_onehot)

        obj_id_onehot = task_id_verbobj_onehot[4:] # TODO: Fix hardcoding
        assert set(obj_id_onehot) == {0, 1} and np.sum(obj_id_onehot) == 1
        meta["obj_ids"] = np.argmax(obj_id_onehot)

        # Load Task index from verb and obj ids
        task_id = self.verb_obj_id_to_task_id_map[(meta["verb_ids"], meta["obj_ids"])]
        meta["task_ids"] = task_id

        meta["onehot_task_ids"] = np.zeros((1, self.env_num_tasks))
        meta["onehot_task_ids"][0][task_id] = 1.0

        if self.load_verb_obj_enc_batch:
            # Load Verb encoder batch
            meta["verb_enc_obs"] = self.get_obs_sequence_from_demo(
                demo_id,
                index_in_demo=0, # Start from beginning of traj
                keys=["robot0_eef_pos", "robot0_gripper_qpos"],
                num_frames_to_stack=self.n_frame_stack - 1,
                seq_length=demo_length,
                prefix="obs",
                sample_size=10, #TODO: Fix this hardcoding
            )

            # Load object encoder batch
            meta['obj_enc_obs'] = self.get_obs_sequence_from_demo(
                demo_id,
                index_in_demo=0, # Start from beginning of traj
                keys=['object'],
                num_frames_to_stack=self.n_frame_stack - 1,
    ...
    def get_sequence_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1, sample_size=None):
        """
        ...
        Args:
            ...
            sample_size (int or None): downsample (sub)sequence to sample_size number of evenly spaced transitions.
        ...
        for k in keys:
            ...
            if sample_size is not None:
                assert seq_begin_index == 0 # start sampling from beginning of traj.
                assert sample_size <= seq_end_index - seq_begin_index - 1
                sampled_idxs = np.round(np.linspace(seq_begin_index, seq_end_index - 1, sample_size)).astype(int)
    ...
    def get_obs_sequence_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1, prefix="obs", sample_size=None):
        """
...
class MultitaskSequenceDataset(SequenceDataset):
    """SequenceDataset, but allows extraction of trajectories of a certain task idx."""
    def __init__(self, *args, **kwargs):
        super().__init__(del_cache_cleanup=False, *args, **kwargs)
        self.task_obs_key = "task_id"
        self.create_verb_obj_id_to_demo_id_list_map()

    def create_verb_obj_id_to_demo_id_list_map(self):
        self.verb_obj_id_to_demo_id_list_map = dict()
        for demo_id in self.demos:
            # Get the task_id
            task_id_verbobj_onehot = self.get_dataset_for_ep(demo_id, f"obs/{self.task_obs_key}")[0] # 0th timestep, since task_id doesn't change during course of demo
            verb_id_onehot = task_id_verbobj_onehot[:4] # TODO: Fix hardcoding
            assert set(verb_id_onehot) == {0, 1} and np.sum(verb_id_onehot) == 1
            obj_id_onehot = task_id_verbobj_onehot[4:] # TODO: Fix hardcoding
            assert set(obj_id_onehot) == {0, 1} and np.sum(obj_id_onehot) == 1
            verb_id = np.argmax(verb_id_onehot)
            obj_id = np.argmax(obj_id_onehot)
            num_objs = len(obj_id_onehot)
            assert isinstance(num_objs, int)

            if (verb_id, obj_id) not in self.verb_obj_id_to_demo_id_list_map:
                self.verb_obj_id_to_demo_id_list_map[(verb_id, obj_id)] = [demo_id]
            else:
                self.verb_obj_id_to_demo_id_list_map[(verb_id, obj_id)].append(demo_id)
        print("self.verb_obj_id_to_demo_id_list_map", dict([(k, len(v)) for k, v in self.verb_obj_id_to_demo_id_list_map.items()]))

    def get_item_matching_verb_obj_id(self, verb_id, obj_id):
        task_demo_ids = self.verb_obj_id_to_demo_id_list_map[(verb_id, obj_id)]
        # choose a task demo id
        demo_id = np.random.choice(task_demo_ids)
        demo_id_start_idx = self._demo_id_to_start_indices[demo_id]
        return self.__getitem__(demo_id_start_idx)
