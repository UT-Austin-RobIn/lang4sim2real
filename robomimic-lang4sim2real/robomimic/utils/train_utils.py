...
from robomimic.utils.dataset import (SequenceDataset, MultitaskSequenceDataset)
...
def get_exp_dir(config, auto_remove_exp_dir=False):
    ...
    auto_remove_exp_dir = auto_remove_exp_dir or "/debug" in base_output_dir
...
def load_data_for_training(config, obs_keys, env_dataset_kwargs={}):
    """
    ...
    if config.experiment.validate:
        ...
        train_dataset = dataset_factory(
            ...
            env_dataset_kwargs=env_dataset_kwargs)
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None, multitask_dataset=False, env_dataset_kwargs={}):
    """
    ...
    ds_kwargs = dict(
        ...
        load_verb_obj_enc_batch=config.algo.verb_obj_enc.enabled,
    )
    ds_kwargs.update(env_dataset_kwargs)

    if multitask_dataset:
        dataset = MultitaskSequenceDataset(**ds_kwargs)
    else:
...
def run_rollout(
    ...
    if env.env.is_multitask_env:
        env.env.set_task_id(task_id)

    ob_dict = env.reset()

    if env.env.is_multitask_env:
        assert env.env.task_id == task_id
        # print("task_id", ob_dict['task_id'])
        assert np.sum(ob_dict['task_id']) == 2 # onehot sums to 2 (concatenation of verb and obj onehots)
        assert set(ob_dict['task_id']) == set([0.0, 1.0]) # onehot only contains 0's and 1's
        # assert ob_dict['task_id'][task_id] == 1
        # assert ob_dict['task_id'][0] == task_id
        # print("ob_dict['task_id']", ob_dict['task_id'])
        if config.algo.verb_obj_enc.enabled:
            task_id_traj_dict = target_taskset.get_item_matching_verb_obj_id(env.env.verb_id, env.env.object_id)

            # Process the obs
            verb_enc_obs = np.concatenate(list(task_id_traj_dict["verb_enc_obs"].values()), axis=-1)
            obj_enc_obs = np.concatenate(list(task_id_traj_dict["obj_enc_obs"].values()), axis=-1)
            task_ids = np.array([task_id_traj_dict["task_ids"]])
        if config.algo.gt_task_emb == "onehot_taskid":
            onehot_task_ids = np.zeros(len(env.env.task_id_to_verb_obj_id_pair_map))
            onehot_task_ids[task_id] = 1.0
    ...
    try:
        for step_i in range(horizon):
            if config.algo.verb_obj_enc.enabled:
                # Add task enc obs (Held constant throughout traj)
                ob_dict["verb_enc_obs"] = verb_enc_obs
                ob_dict["obj_enc_obs"] = obj_enc_obs

                if config.algo.verb_obj_enc.task_emb_mode == "lang":
                    ob_dict['task_ids'] = task_ids

            if config.algo.gt_task_emb == "onehot_taskid":
                ob_dict["task_id"] = onehot_task_ids
            ...
            for k in success:
                ...
                # We don't want to smear success rate over time;
                # only take success at latest timestep
                success[k] = cur_success_metrics[k]
    ...
    for k in success:
        new_key = f"{k}_Success_Rate"
        if k != "task":
            results[new_key] = float(success[k])
...
def rollout_with_stats(
    ...
    for env_name, env in envs.items():
        ...
        for ep_i in iterator:
            if env.env.is_multitask_env:
                assert num_episodes % env.env.num_tasks == 0
                num_eval_rollouts_per_task = num_episodes // env.env.num_tasks
                task_id = ep_i // num_eval_rollouts_per_task
                horizon = env.env.get_horizon_from_task_id(task_id)
            else:
                task_id = None
        ...
        # Normalize task success rates by task_ID
        for task_id in range(env.env.num_tasks):
            potential_key_to_normalize = f"task_{task_id}_Success_Rate"
            if potential_key_to_normalize in rollout_logs_mean:
                rollout_logs_mean[potential_key_to_normalize] *= env.env.num_tasks

        # Calculate task success rates by train and test groups
        if len(test_task_ids) > 0:
            rollout_logs_mean["Test_Tasks_Success_Rate"] = np.mean(
                [rollout_logs_mean[f"task_{test_task_id}_Success_Rate"]
                for test_task_id in test_task_ids])
        if len(train_task_ids) > 0:
            rollout_logs_mean["Train_Tasks_Success_Rate"] = np.mean(
                [rollout_logs_mean[f"task_{train_task_id}_Success_Rate"]
