...
def get_all_params_from_net_list(nets):
    """
    Returns list of all parameters from `nets`, which is either an
    nn.Module or a list of such things.
    """
    if isinstance(nets, list):
        params_to_update = []
        for net in nets:
            params_to_update.extend(list(net.parameters()))
    else:
        params_to_update = nets.parameters()
...
def optimizer_from_optim_params(net_optim_params, net):
    ...
    """
    return optim.Adam(
        params=get_all_params_from_net_list(net),
...
def backprop_for_loss(net, optim, loss, max_grad_norm=None, retain_graph=False):
    ...
    Args:
        net (torch.nn.Module or a List of these): network to update
    ...
    # compute grad norms
    grad_norms = 0.
    params_to_update = get_all_params_from_net_list(net)
