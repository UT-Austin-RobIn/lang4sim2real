...
class SingleArm(Manipulator):
    ...
    def setup_observables(self):
        ...
        if self.has_gripper:
            ...
            @sensor(modality=modality)
            def gripper_states(obs_cache):  # 1-dim summary state of gripper openness.
                gripper_qpos = np.array([
                    self.sim.data.qpos[x] for x in self._ref_gripper_joint_pos_indexes])
                assert gripper_qpos.shape[0] == 2
                gripper_qpos_diff = np.array([np.max(gripper_qpos) - np.min(gripper_qpos)])
                # return -gripper_qpos_diff / 0.08
                return gripper_qpos_diff

            sensors += [gripper_qpos, gripper_qvel, gripper_states]
            names += [f"{pf}gripper_qpos", f"{pf}gripper_qvel", f"{pf}gripper_states"]
