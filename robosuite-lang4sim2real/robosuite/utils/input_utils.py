...
from robosuite.policies import *
...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    """
    controller_output = device.get_controller_state()
    action, grasp = controllerOutput2action(controller_output, device, robot, active_arm, env_configuration)
    return action, grasp


def scriptedPolicy2action(policy, env_obs, robot, active_arm="right", env_configuration=None):
    """
    Same as input2action but uses a scripted policy instead of device, where the policy is conditioned
    on the env_obs instead of any human inputs.
    """
    controller_output, info = policy.get_action(env_obs)
    action, grasp = controllerOutput2action(controller_output, device, robot, active_arm, env_configuration)
    return action, grasp, info


def controllerOutput2action(controller_output, device, robot, active_arm, env_configuration):
    """Processes controller output into an action for the robot."""
    ...
    dpos, rotation, raw_drotation, grasp, reset = (
        controller_output["dpos"],
        controller_output["rotation"],
        controller_output["raw_drotation"],
        controller_output["grasp"],
        controller_output["reset"],
    )
