...
class BaseModder:
    ...
    def get_param_limits(self, param_idx):
        ...
        """
        return self.param_name_to_hilo_range[self.param_idx_to_name[param_idx]]
    ...
    def update_param_range(self, param_idx, new_range):
        ...
        self.param_name_to_hilo_range[self.param_idx_to_name[param_idx]] = new_range
...
class LightingModder(BaseModder):
    ...
    def __init__(
        ...
        # For Automatic domain randomization, store the ranges
        self.param_idx_to_name = ["pos", "dir", "specular", "ambient", "diffuse"]
        self.num_params = len(self.param_idx_to_name)
        self.param_name_to_hilo_range = dict(
            pos=[-1 * self.position_perturbation_size, self.position_perturbation_size],
            dir=[None, self.direction_perturbation_size],
            specular=[-1 * self.specular_perturbation_size, self.specular_perturbation_size],
            ambient=[-1 * self.ambient_perturbation_size, self.ambient_perturbation_size],
            diffuse=[-1 * self.diffuse_perturbation_size, self.diffuse_perturbation_size],
        )

        self.param_name_to_setter_fn = dict(
            pos=self.set_pos,
            dir=self.set_dir,
        ...
        self.param_name_to_getter_fn = dict(
            pos=self.get_pos,
            dir=self.get_dir,
    ...
    def set_param(self, param_idx, delta_val):
        setter_fn = self.param_name_to_setter_fn[self.param_idx_to_name[param_idx]]
        for light_name in self.light_names:
            param_name = self.param_idx_to_name[param_idx]
            if param_name == "dir":
                val = self.calc_dir_from_angle(light_name, delta_val)
            else:
                val = self._defaults[light_name][param_name] + delta_val
            setter_fn(light_name, val)
    ...
    def _randomize_position(self, name):
        ...
        """
        lo, hi = self.param_name_to_hilo_range["pos"]
    ...
    def _randomize_direction(self, name):
        ...
        _, hi = self.param_name_to_hilo_range["dir"]
        random_axis, random_angle = trans.random_axis_angle(
            angle_limit=hi, random_state=self.random_state
        )
    ...
    def _randomize_specular(self, name):
        ...
        Args:
            name (str): Name of the lighting source to randomize for
        """
        lo, hi = self.param_name_to_hilo_range["specular"]
    ...
    def _randomize_ambient(self, name):
        ...
        Args:
            name (str): Name of the lighting source to randomize for
        """
        lo, hi = self.param_name_to_hilo_range["ambient"]
        delta = self.random_state.uniform(
            low=lo,
            high=hi,
            size=3,
    ...
    def _randomize_diffuse(self, name):
        ...
        Args:
            name (str): Name of the lighting source to randomize for
        """
        lo, hi = self.param_name_to_hilo_range["diffuse"]
        delta = self.random_state.uniform(
            low=lo,
            high=hi,
            size=3,
    ...
    def calc_dir_from_angle(self, name, angle):
        random_axis = np.random.randn(3)
        random_axis /= np.linalg.norm(random_axis)
        random_delta_rot = trans.quat2mat(trans.axisangle2quat(random_axis * angle))
...
class CameraModder(BaseModder):
    ...
    def __init__(
        ...
        self.param_idx_to_name = ["pos", "quat", "fovy"]
        self.num_params = len(self.param_idx_to_name)
        self.param_name_to_hilo_range = dict(
            pos=[-self.position_perturbation_size, self.position_perturbation_size],
            quat=[None, self.rotation_perturbation_size],
            fovy=[-self.fovy_perturbation_size, self.fovy_perturbation_size],
        )
        self.param_name_to_setter_fn = dict(
            pos=self.set_pos,
            quat=self.set_quat,
    ...
    def set_param(self, param_idx, delta_val):
        ...
        for name in self.camera_names:
            param_name = self.param_idx_to_name[param_idx]
            if param_name == "quat":
                val = self.calc_quat_from_angle(name, delta_val)
            else:
                val = self._defaults[name][param_name] + delta_val
            setter_fn(name, val)
    ...
    def _randomize_rotation(self, name):
        ...
        Args:
            name (str): Name of the camera to randomize for
        """
        _, hi = self.param_name_to_hilo_range["quat"]
        # sample a small, random axis-angle delta rotation
        random_axis, random_angle = trans.random_axis_angle(
            angle_limit=hi, random_state=self.random_state
        )
    ...
    def _randomize_fovy(self, name):
        ...
        Args:
            name (str): Name of the camera to randomize for
        """
        lo, hi = self.param_name_to_hilo_range["fovy"]
    ...
    def calc_quat_from_angle(self, name, angle):
        # sample a small, random axis-angle delta rotation
        random_axis = np.random.randn(3)
        random_axis /= np.linalg.norm(random_axis)
        random_delta_rot = trans.quat2mat(trans.axisangle2quat(random_axis * angle))
...
class DynamicsModder(BaseModder):
    ...
    def __init__(
        ...
        # For Automatic domain randomization, store the ranges
        # We do not want to randomize certain params because they are more
        # complicated to set.
        self.param_idx_to_name = []
        if randomize_density:
            self.param_idx_to_name.append("density")
        if randomize_viscosity:
            self.param_idx_to_name.append("viscosity")
        if randomize_position:
            self.param_idx_to_name.append("position")
        if randomize_quaternion:
            self.param_idx_to_name.append("quaternion")
        if randomize_inertia:
            self.param_idx_to_name.append("inertia")
        if randomize_mass:
            self.param_idx_to_name.append("mass")
        if randomize_friction:
            self.param_idx_to_name.append("friction")
        if randomize_solref:
            self.param_idx_to_name.append("solref")
        if randomize_solimp:
            self.param_idx_to_name.append("solimp")
        if randomize_stiffness:
            self.param_idx_to_name.append("stiffness")
        if randomize_frictionloss:
            self.param_idx_to_name.append("frictionloss")
        if randomize_damping:
            self.param_idx_to_name.append("damping")
        if randomize_armature:
            self.param_idx_to_name.append("armature")
        # self.param_idx_to_name = [
        #     # Opt parameters
        #     "density",
        #     "viscosity",
        ...
        self.num_params = len(self.param_idx_to_name)
        self.param_name_to_hilo_range = dict()
        for rand_setting_dict in [
                self.opt_randomizations, self.body_randomizations,
                self.geom_randomizations, self.joint_randomizations]:
            for key in rand_setting_dict:
                self.param_name_to_hilo_range[key] = [
                    -rand_setting_dict[key]["perturbation"],
                    rand_setting_dict[key]["perturbation"]]
        self.save_defaults()
    ...
    def set_param(self, param_idx, delta_val):
        ...
        for group_defaults, group_randomizations, group_randomize_names in zip(
            ...
            if param_name in group_defaults[group_randomize_names[0]].keys():
                for name in group_randomize_names:
                    # Randomize all parameters associated with this element
                    param_name = self.param_idx_to_name[param_idx]
                    default_val = group_defaults[name][param_name]
                    val = copy.copy(default_val)
                    settings = group_randomizations[param_name]
                    if settings["randomize"]:
                        val = val + delta_val if settings["type"] == "size" else val * (1.0 + delta_val)
                        val = np.clip(val, *settings["clip"])
                    # Modify this value
                    self.mod(name=name, attr=param_name, val=val)
                    param_set = True

        assert param_set, "No params set when calling set_param(...)"
            ...
            for name in group_randomize_names:
                ...
                for attr, default_val in group_defaults[name].items():
                    ...
                    if settings["randomize"]:
                        ...
                        lo, hi = self.param_name_to_hilo_range[attr]
                        perturbation = (
                            np.random.uniform(low=lo, high=hi) if type(val) in {int, float}
                            else np.random.uniform(low=lo, high=hi, size=val.shape))
