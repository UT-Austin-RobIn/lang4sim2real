...
class UniformRandomSampler(ObjectPositionSampler):
    ...
    def sample(self, fixtures=None, reference=None, on_top=True, objs_on_scene=None):
        """
        ...
        Args:
            ...
            objs_on_scene (Union[List, None]): If provided, only sample over these objects.
        ...
        if objs_on_scene is None:
            objs_on_scene = self.mujoco_objects

        for obj in objs_on_scene:
        ...
        for sampler, s_args in zip(self.samplers.values(), self.sample_args.values()):
            ...
            for arg_name, arg in zip(("reference", "on_top"), (reference, on_top)):
                if arg_name not in s_args:
                    s_args[arg_name] = arg
            if sampler.name == 'CollisionObjectSampler':
                s_args['objs_on_scene'] = objs_on_scene

            # Run sampler
            new_placements = sampler.sample(
                fixtures=placed_objects, **s_args)
            # Update placements
