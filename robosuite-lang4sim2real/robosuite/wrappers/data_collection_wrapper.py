...
class DataCollectionWrapper(Wrapper):
    def __init__(self, env, directory, collect_freq=1, flush_freq=1000, allow_prints=True):
        """
        ...
        self.states = []
        self.goal = None # stores info like task_ids
        ...
        # whether to allow print statements
        self.allow_prints = allow_prints

        if not os.path.exists(directory):
            if self.allow_prints: print("DataCollectionWrapper: making new directory at {}".format(directory))
    ...
    def _start_new_episode(self, save_traj):
        """
    ...
    def _on_first_interaction(self):
        ...
        if self.allow_prints: print("DataCollectionWrapper: making folder at {}".format(self.ep_directory))
    ...
    def _flush(self, save_traj=True):
        """
        ...
        if save_traj:
            ...
            if self.goal is not None:
                save_kwargs['goal'] = np.array(self.goal)
                # print("saved goal", save_kwargs['goal'])
            np.savez(state_path, **save_kwargs)
        self.states = []
        # self.goal = None
        self.action_infos = []

    def reset(self, save_traj=True):
        """
        ...
        """
        self._start_new_episode(save_traj)
        ret = super().reset()
        return ret
