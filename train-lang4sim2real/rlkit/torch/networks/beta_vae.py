...
from rlkit.torch.networks.base_vae import BaseVAE
from rlkit.util.types import List, Tensor
...
class BetaVAE(BaseVAE):
    ...
    def __init__(self,
        ...
        self.final_layer = nn.Sequential(
            nn.ConvTranspose2d(hidden_dims[-1],
                               ...
                               output_padding=int(kernel_sizes[-1] // 2)),
    ...
    def encode(self, _input: Tensor) -> List[Tensor]:
        """
    ...
    def forward(self, _input: Tensor, **kwargs) -> Tensor:
        mu, log_var = self.encode(_input)
        z = self.reparameterize(mu, log_var)
        return [self.decode(z), _input, mu, log_var]
    ...
    def loss_function(self,
        ...
        if self.loss_type == 'H': # https://openreview.net/forum?id=Sy2fzU9gl
            ...
            self.C_max = self.C_max.to(_input.device)
            C = torch.clamp(
