...
def parse_args():
    ...
    parser.add_argument(
        "--interface-cfg", type=str, default="alice.yml")
    parser.add_argument(
        "--controller-cfg", type=str, default="osc-controller.yml")
    parser.add_argument(
        "--folder", type=Path, default="example_data")
...
def main():
    ...
    for (run_idx, path) in enumerate(Path(folder).glob("run*")):
        ...
        action_data = np.load(
            f"{path}/testing_demo_action.npz", allow_pickle=True)[
                "data"]
        ...
        for camera_id in camera_ids:
            camera_data[camera_id] = np.load(
                f"{path}/testing_demo_camera_{camera_id}.npz",
                allow_pickle=True
            )["data"]
        ...
        for i in range(len_data):
            ...
            for camera_id in camera_ids:
                ...
                if "color" in camera_data[camera_id][i]:
                    ...
                    image_color_data[camera_id].append(
                ...
                if "depth" in camera_data[camera_id][i]:
                    ...
                    image_depth_data[camera_id].append(
                        resized_img.transpose(2, 0, 1))
        ...
        proprio_gripper_state_list.append(
            np.stack(proprio_gripper_state, axis=0))

        for camera_id in camera_ids:
            ep_data_grp = ep_grp.create_dataset(
                f"camera_{camera_id}_color",
                data=image_color_names_data[camera_id]
            )
        ...
        ep_data_grp = ep_grp.create_dataset(
            "proprio_ee", data=np.stack(proprio_ee))
        ep_data_grp = ep_grp.create_dataset(
